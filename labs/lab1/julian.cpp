//  Program:  Convert to Julian Day
//  Author:   Chuck Stewart
//  Purpose:  Convert a day and month within a given year to the
//    Julian day. 
...
main()
{
  int months[10], days[10], julian_days[10];
  for (int i = 0; i < 10; i++) {
      cout << "Please enter two integers (month and day): "; 
      cin >> months[i] >> days[i];
  }

  for (int i = 0; i < 10; i++)
      julian_days[i] = julian_day(months[i], days[i], 2012);

  for (int i = 0; i < 10; i++)
      cout << "Julian day for " << days[i] << " " << months[i] << " 2102 is "
          << julian_days[i] << endl;
