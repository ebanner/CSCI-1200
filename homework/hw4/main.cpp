...
void InsertStudentIntoSchoolPreferenceList(std::istream &istr, std::list<School> &schools) {
   ...
   for(itr = schools.begin(); itr->GetSchoolName() != school_name; itr++)
      ; // ride until you get to the school with the school name we want

   // add the student to the school's preference list
   itr->AddStudent(new_student_name, insert_before_student_name);
}
...
void RemoveSchoolFromStudentPreferenceList(std::istream &istr, std::list<Student> &students) {
   ...
   /* We need to find the Student in the list of students that has the name
    * `student_name'. */
   list<Student>::iterator itr;
   for (itr = students.begin(); itr->GetStudentName() != student_name; itr++)
      ; // ride along until we find the Student with `student_name'

   // remove the school from the student's preference list
   itr->RemoveFromSchoolPreference(school_name);
}
