...
template <class T> void my_print(const MultiLL<T> &multill)
{
  std::cout << "MutliLL is now (chronological):" << std::endl;
  typename MultiLL<T>::iterator itr;
  for (itr = multill.begin_chronological(); itr != multill.end_chronological(); itr++)
    std::cout << *itr << std::endl;
  std::cout << std::endl;
  std::cout << "MutliLL is now (sorted):" << std::endl;
  for (itr = multill.begin_sorted(); itr != multill.end_sorted(); itr++)
    std::cout << *itr << std::endl;
  std::cout << std::endl;
}
